...
function navbarAnimation() {
    ...
    gsap.to("#name", {
        ...
        scrollTrigger: {
            ...
            scrub: 3,
            markers: true // Remove or comment out for production
        }
    });
}
...
const observer = new IntersectionObserver(entries => {
    entries.forEach(entry => {
        if (entry.isIntersecting || entry.intersectionRatio > 0) {
            ...
            observer.unobserve(entry.target); // Remove observer after element is shown
        }
    });
}, {
    threshold: 0.5,
    rootMargin: '0px 0px -50px 0px' // Adjust bottom margin as needed
});
...
document.getElementById('login-button').addEventListener('click', function() {
    window.location.href = 'https://www.maitilabs.org/login';
});
document.getElementById('join-button').addEventListener('click', function() {
    window.location.href = 'https://chat.whatsapp.com/HToo6tS5XFO0bxbcOQTdy7';
});

document.getElementById('awareness').addEventListener('click', function() {
    window.location.href = 'https://www.maitilabs.org/awareness';
});
